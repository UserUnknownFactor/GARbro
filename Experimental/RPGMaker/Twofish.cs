// MIT License
// Author: Josip Medved
// Algorithm designer: Bruce Schneier
...
// Changed from 'file' to 'internal' for C# 7.3
...
/// </summary>
internal sealed class TwofishTransform : ICryptoTransform
{
    ...
    {
        if (rgbKey == null) { throw new ArgumentNullException(nameof(rgbKey), "Key cannot be null."); }
        if (rgbKey.Length != 16 && rgbKey.Length != 24 && rgbKey.Length != 32) { throw new ArgumentOutOfRangeException(nameof(rgbKey), "Key must be 128, 192, or 256 bits."); }
        if ((rgbIV != null) && (rgbIV.Length != 16)) { throw new ArgumentOutOfRangeException(nameof(rgbKey), "IV must be 128 bits."); }
        ...
        SBoxKeys = new DWord[MaxKeyBits / 64];  // key bits used for S-boxes
        SubKeys = new DWord[TotalSubkeys];  // round subkeys, input/output whitening bits
        ...
        if (TransformMode == TwofishTransformMode.Encrypt)
        {
            ...
            switch (PaddingMode)
            {
                ...
                case PaddingMode.ISO10126:
                    ...
                    using (var rng = RandomNumberGenerator.Create())
                    {
                        var randomBytes = new byte[paddedLength - inputCount - 1];
                        rng.GetBytes(randomBytes);
                        Buffer.BlockCopy(randomBytes, 0, paddedInputBuffer, inputCount, randomBytes.Length);
                    }
                    paddedInputOffset = 0;
                    Buffer.BlockCopy(inputBuffer, inputOffset, paddedInputBuffer, 0, inputCount);
                    paddedInputBuffer[paddedInputBuffer.Length - 1] = (byte)(paddedLength - inputCount);
    ...
    private readonly DWord[] IV = new DWord[16];
    ...
    private static readonly object SyncRootBuildMds = new object();
