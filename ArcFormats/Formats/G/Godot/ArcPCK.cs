...
namespace GameRes.Formats.Godot
{
    ...
    public class GodotEncryptedFileStream : Stream
    {
        ...
        public GodotEncryptedFileStream(Stream input, byte[] key, bool leaveOpen = false)
        {
            ...
            // Create decryption stream for the remaining data
    ...
    public class PckOpener : ArchiveFormat
    {
        ...
        public PckOpener()
        {
            ...
            Signatures = new uint[] { PCK_MAGIC, 0 };
        }
        ...
        public override ArcFile TryOpen(ArcView file)
        {
            ...
            // Check if it's an executable with embedded PCK
            if (signature != PCK_MAGIC)
            {
                if (!IsExecutable(signature))
                    return null;

                var pckOffset = SearchForEmbeddedPck(file);
                if (pckOffset < 0)
                    return null;

                baseOffset = pckOffset;
                signature = file.View.ReadUInt32(baseOffset);
                if (signature != PCK_MAGIC)
                    return null;
            }
            ...
            try
            {
                return ReadPckArchive(file, baseOffset);
            }
            catch (Exception ex)
            {
                System.Diagnostics.Trace.WriteLine($"Failed to read PCK archive: {ex.Message}");
                return null;
            }
        }

        private static bool IsExecutable(uint signature)
        {
            return signature == 0x00905A4D || // MZ header
                   signature == 0x0000005A || // Possible MZ variant
                   signature == 0x464C457F;   // ELF header
        }
        ...
        private PckHeader ReadPckHeader(BinaryReader reader)
        {
            ...
            if (header.FormatVersion >= 2)
            {
                ...
                // AFTER reading flags/filebase/diroffset, skip 16 reserved dwords
        ...
        private long SearchForEmbeddedPck(ArcView file)
        {
            // Search for embedded PCK from the end of file
            const int SEARCH_BUFFER_SIZE = 65536;
            const int EMBEDDED_SIGNATURE_SIZE = 12; // 8 bytes offset + 4 bytes 'GDPC'

            long fileSize = file.MaxOffset;
            byte[] buffer = new byte[SEARCH_BUFFER_SIZE];

            for (long pos = fileSize - EMBEDDED_SIGNATURE_SIZE; pos > 0; pos -= SEARCH_BUFFER_SIZE - EMBEDDED_SIGNATURE_SIZE)
            {
                long readPos = Math.Max(0, pos - SEARCH_BUFFER_SIZE + EMBEDDED_SIGNATURE_SIZE);
                int readSize = (int)Math.Min(SEARCH_BUFFER_SIZE, fileSize - readPos);

                if (file.View.Read(readPos, buffer, 0, (uint)readSize) != readSize)
                    continue;

                // Search for 'GDPC' signature in buffer
                for (int i = readSize - 4; i >= 0; i--)
                {
                    if (BitConverter.ToUInt32(buffer, i) == PCK_MAGIC)
                    {
                        // Found potential end marker, verify by reading the offset
                        long markerPos = readPos + i;
                        if (markerPos >= 8)
                        {
                            long pckSize = file.View.ReadInt64(markerPos - 8);
                            long pckStart = markerPos - pckSize - 8;

                            if (pckStart >= 0 && pckStart < markerPos)
                            {
                                // Verify PCK header at calculated position
                                if (file.View.ReadUInt32(pckStart) == PCK_MAGIC)
                                    return pckStart;
                            }
                        }
                    }
